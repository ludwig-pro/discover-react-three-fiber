  ...
  if (child) {
    if (child.isObject3D) {
      ...
      if (child.attachArray) parentInstance[child.attachArray] = parentInstance[child.attachArray].filter(function (x) {
        return x !== child;
      });else if (child.attachObject) delete parentInstance[child.attachObject[0]][child.attachObject[1]];else if (child.attach) parentInstance[child.attach] = null;
    }
...
function render(element, container, state) {
  ...
  if (!root) {
    ...
    var newRoot = root = Renderer.createContainer(container, state !== undefined && state.current.concurrent, false);
